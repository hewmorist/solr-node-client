...
/**
 * Filter the set of documents found before to return the result with the given `field` and ORed `values`.
 *
 * @param {String} field - name of field
 * @param {Array of strings} value - Array of values of the field that can match
 *
 * @return {Query}
 * @api public
 *
 * @example
 * var query = client.createQuery();
 * query.q({ '*' : '*' }).orFilter('id', ["1", "100" "1000"])
 */

Query.prototype.orFilter = function(field,value){
   var self = this;
   var parameter = 'fq=';
   var orstring = '';
   
   _.each(value, function(strg) {
       orstring += strg+' OR ';
   ...
   parameter += field + ':' + encodeURIComponent(orstring);
   this.parameters.push(parameter);
   return self;
};
